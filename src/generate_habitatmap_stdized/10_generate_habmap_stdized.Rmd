...
Interim version of the habitatmap 3/2025
...
To be sure we will use the correct version of the data source (habitatmap_2025_interim), 
...
Are there NA? Yes, as expected
```{r check_na}
sapply(habitatmap_sf, function(x) sum(is.na(x)))
```

Are there <Null>? No, as expected.
```{r check_null}
sapply(habitatmap_sf |> st_drop_geometry(), function(x) sum(as.character(x) == '<Null>', na.rm = TRUE))
```
Are there empty strings? No, as expected.

```{r check_empty_string}
sapply(habitatmap_sf |> st_drop_geometry(), function(x) sum(as.character(x) == '', na.rm = TRUE))
```

Are there spaces? No, as expected.

```{r check_space}
sapply(habitatmap_sf |> st_drop_geometry(), function(x) sum(grepl(pattern = "^\\s+$", x), na.rm = TRUE))
```
