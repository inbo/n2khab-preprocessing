...
We rename some column names to make the interim version as similar as possible to the regular one; this prevents unneeded differences with the regular version in later code chunks.
Also, we clean strings and add a unique polygon ID.
...
habmap_longHAB %>%
  distinct(code) %>%
  arrange(code) %>%
  kable(caption = "Codes in habitat map", label = NA) %>%
  kable_styling() %>%
...
`habmap_types` is the object used to write the table of type occurrences in the datasource.
...
leaflet(height = "600px", width = "700px") %>%
  ...
  addPolygons(
    data = habmap_types_geom_invalid_wgs84,
...
habmap_types_geom_corrected %>%
  ...
  kable(
    caption = "Polygons with faulty geometries after correction",
    label = NA
  ) %>%
  kable_styling() %>%
  scroll_box(height = "300px")
...
leaflet(height = "600px", width = "700px") %>%
  ...
  addPolygons(
    data = habmap_types_geom_corrected_wgs84,
...
<!-- BELOW TEMPORARY STEP IS TO BE DROPPED IN LATER UPDATES (SHOULD HAVE BEEN FIXED IN DATABASE): -->

Temporary fixes of unsupported values (this affects two rows):

```{r}
habmap_types_sf <-
  habmap_types_sf %>%
  mutate(source_hab = replace_values(source_hab, c("24m", "24hab") ~ "24mk"))
```
