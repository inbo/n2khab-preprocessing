...
+ To be sure we will use the correct version of the data source (GRTSmh_base4frac_v1), we will derive the md5 file hashes and compare it to the file hashes in the [data source version overview table](https://docs.google.com/spreadsheets/d/1E8ERlfYwP3OjluL8d7_4rR1W34ka4LRCE35JTxf3WMI/edit#gid=2100595853).

```{r}
path <- fileman_up("n2khab_data")
file <- "20_processed/GRTSmh_base4frac"
...
hashes <-
    ...
    mutate(filename = str_match(filepath, "(.+\\/)*(.+)")[,3],
           ...
           md5_ref = c("723ece9ec6e77a7dc3928ee1872040ab"),
           match = md5 == md5_ref) %>%
    select(filename,
           md5,
           md5_ref,
...
+ Finally we mask the raster using a shapefile of Flanders with a buffer of 1024 m: all raster cells outside the buffer get a NA value.
...
flanders_buffer <- read_admin_areas(dsn = "flanders") %>%
  st_buffer(32 * 32)

raster_runif_buffer_mask <- raster_runif_buffer %>%
  crop(flanders_buffer) %>%
  mask(flanders_buffer)
...
sampling_units_example <- grts_habpolygon_example %>%
  mutate(draw_runif = raster_runif_buffer[as(., "Spatial")])%>%
