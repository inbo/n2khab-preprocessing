...
The following polygons have an invalid geometry.
...
geom_validity_habmap_corrected <- 
  st_is_valid(habitatmap_21xx_polygons %>% 
                   st_transform(crs = 4326), 
              reason = TRUE) %>% 
  as_tibble() %>% 
  ...
  left_join(habitatmap_21xx_polygons %>% 
              ...
              mutate(id = as.character(row_number())), 
            by = "id") %>% 
  filter(polygon_id %in% geom_validity_habmap$polygon_id)

kable(geom_validity_habmap_corrected %>% 
        select(id, value, polygon_id, description_orig, geom)) %>% 
  kable_styling()
```
...
geometry_valid <- nrow(geom_validity_ws_standw) == 0
