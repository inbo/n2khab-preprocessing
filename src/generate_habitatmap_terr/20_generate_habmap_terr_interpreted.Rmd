...
checksums <-
  ...
  mutate(
    ...
    md5_ref = c("6cc9dc73f316fbaa353fe11ff33f3359"),
    match = md5 == md5_ref
  ) %>%
  select(
    filename,
...
Again we will check if it is the correct version (habitatmap_2024_v99_interim).
...
```{r read_raw_data, message=FALSE, eval=FALSE}
# official version:
...
# interim version:
habmap_sf <- st_read(file.path(path, file, "habitatmap.gpkg"),
                  crs = 31370)
...
habmap_sf <- habmap_sf %>% 
    mutate(across(-geom, ~ifelse(grepl(pattern = "^\\s*$", .), NA, .)),
           info = ifelse(info == '<Null>', NA, info)) %>% # replace empty fields by NA
    mutate(polygon_id = globalid_BWK, # add unique ID (we replace TAG by the unique globalid_BWK)
           bwk_label = gsub(pattern = "\\s\\+\\sNA", 
                           replacement = "",
