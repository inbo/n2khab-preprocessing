...
hashes <-
    ...
    filter(!str_detect(filepath, "habitatmap_stdized_2020|habitatmap_stdized_2018")) %>%
    # in case other version are stored in a subfolder
    mutate(filename = str_match(filepath, "(.+\\/)*(.+)")[,3],
           md5 = map(filepath, function(x) {
                           file(x) %>% md5 %>% str_c(collapse = '')
                         }) %>% as.character,
           md5_ref = c("29ac23d878ad9ec337699d3c88ae45b0"),
           match = md5 == md5_ref) %>%
    select(filename,
           md5,
           md5_ref,
...
As the `watersurfaces_hab` data source also covers (semi-)terrestrial areas (see introduction), it is not such a good idea to use that data source and consider all the areas overlapping with `watersurfaces_hab` as stricly aquatic.^[
