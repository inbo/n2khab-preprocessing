...
habmap_certain <- habmap_patches_interpreted %>%
    filter(!str_detect(code_orig, "bos") & !(code_orig %in% c("6510,gh", "9120,gh")))
habmap_polygons_interpreted <- habmap_polygons_interpreted %>%
    semi_join(habmap_certain)
...
habmap_polygons_opp <- habmap_patches_interpreted %>% 
    group_by(polygon_id) %>% 
    summarise(oppsum = sum(phab))

habmap_polygons_water <- habmap_patches_interpreted %>% 
    inner_join(habmap_polygons_opp) %>% 
