...
# Read standardized habitatmap and raw habitatmap

## Data sources

Startingpoint for the interpretation is the interim version of the standardized habitat map of Flanders. For more details about this interim version see [its source location at Zenodo](https://doi.org/10.5281/zenodo.16876755).

### Standardized version

To be sure we will use the correct version of the data source (habitatmap_stdized_2025_v99_interim), we will derive the md5 file checksums and compare it to the file checksums in the [data source version overview table](https://docs.google.com/spreadsheets/d/1E8ERlfYwP3OjluL8d7_4rR1W34ka4LRCE35JTxf3WMI/edit#gid=2100595853).
...
checksums <-
  ...
  mutate(
    ...
    md5_ref = c("46b816b8386ec1d1205dbc5fc139877d"),
    match = md5 == md5_ref
  ) %>%
  select(
    filename,
...
if (!all.equal(checksums$md5, checksums$md5_ref)) {
  stop("The standardized version is NOT up to date ! Please check the datasource.")
}
```


```{r read_processed_data}
# this chunk requires n2khab to support the applicable version of
# habitatmap_stdized
habmap_stdized <- read_habitatmap_stdized(
  version = "habitatmap_stdized_2025_v99_interim"
)
habmap_polygons <- habmap_stdized$habitatmap_polygons
habmap_types <- habmap_stdized$habitatmap_types
```

### Raw data source version

In some cases we'll need to refer to the raw data source 'habitatmap'.
Its interim version is not published and will be imported manually.

Again we will check if it is the correct version (habitatmap_2025_v99_interim).
...
if (!all.equal(checksums$md5, checksums$md5_ref)) {
  stop("The source map is NOT up to date ! Please check the datasource.")
}
```


```{r read_raw_data_interim_cleanup}
# below code is largely borrowed from the generate_habitatmap_stdized project in
# the interim_versions branch; the only original addition here is the definition
# of a bwk_label column
habmap_sf <- 
...
habmap_sf <- habmap_sf %>%
  ...
  mutate(
    ...
    bwk_label = gsub(
      pattern = "\\s\\+\\sNA",
      replacement = "",
      paste(eenh1, eenh2, eenh3, eenh4, eenh5, eenh6, eenh7, eenh8, sep = " + ")
    )
  ) %>%
...
habmap_polygons_interpreted <- habmap_polygons %>%
  ...
  mutate(
    description = ifelse(
      is.na(rbbhfl),
      description_orig,
      str_replace_all(description_orig, "rbbhf", "rbbhfl")
    ),
    source = ifelse(
      is.na(rbbhfl),
      "habitatmap_stdized",
...
habmap_types_interpreted <- habmap_types %>%
  ...
  mutate(
    type = ifelse(
      is.na(rbbhfl) | type != "rbbhf",
      as.character(type),
      str_replace_all(type, "rbbhf", "rbbhfl")
    ),
    source = ifelse(
      is.na(rbbhfl) | type != "rbbhfl",
      "habitatmap_stdized",
      "habitatmap_stdized + interpretation"
    )
  ) %>%
...
Note that it's possible that a watersurface is being mapped together with other biotopes. Since the first unit of a complex is most probably predominant, 2190 will be only be interpreted as 2190_a if the first BWK type is lentic.
...
# update habmap_polygons_interpreted
habmap_polygons_interpreted <- habmap_polygons_interpreted %>%
  ...
  mutate(
    # safeguard mappings on subtype-level from changes:
    description = ifelse(
      is.na(dunewater),
    ...
    source = ifelse(
      str_detect(description, "2190") | source == "habitatmap_stdized + interpretation",
      "habitatmap_stdized + interpretation",
      "habitatmap_stdized"
    ),
    description = str_replace_all(description, "2191", "2190")
  ) %>%
  select(-dunewater)

# update habmap_types_interpreted
habmap_types_interpreted <- habmap_types_interpreted %>%
  ...
  mutate(
    # safeguard mappings on subtype-level from changes:
    type = str_replace_all(type, "2190_", "2191_"), 
    type = case_when(
    ...
    type = str_replace_all(type, "2191_", "2190_")
  ) %>%
  select(-dunewater)
```
...
habmap_polygons_interpreted <-
  habmap_polygons_interpreted %>%
  semi_join(habmap_types_interpreted, join_by(polygon_id))
```

## Exclude polygons exclusively represented by aquatic types
...
watertypes <- data.frame("watertype" = c(
  "1130",
  "2190_a",
  "3110",
  "3130",
...
habmap_polygons_water <-
  habmap_types_interpreted %>%
  mutate(
    is_watertype = type %in% watertypes$watertype,
    polhab_is_exclusive_watertypes = all(is_watertype),
    .by = polygon_id
  ) %>%
  filter(polhab_is_exclusive_watertypes) %>%
  distinct(polygon_id)
...
Additionally, if 6230 has been extracted from the original code "6230,6410", it is considered as subtype 6230_hmo.
...
habmap_polygons_interpreted <- habmap_polygons_interpreted %>%
  mutate(
    ...
    description = str_replace_all(description, c(
      "6230,6410" = "6230_hmo,6410",
      "6410,6230" = "6410,6230_hmo",
      "6410" = "6410_mo",
      "6430" = "6430_hf",
      "6510" = "6510_hu",
      "7140" = "7140_meso",
    ...
    description = str_replace_all(description, "µ_", "0_"),
...
habmap_types_interpreted <- habmap_types_interpreted %>%
  mutate(
    source = ifelse(
      type %in% c("6410", "6430", "6510", "7140", "9130") |
        (code_orig %in% c("6410,6230", "6230,6410") & type == "6230"),
      "habitatmap_stdized + interpretation",
      source
    ),
    type = replace_values(type,
      "6410" ~ "6410_mo",
      "6430" ~ "6430_hf",
      "6510" ~ "6510_hu",
      "7140" ~ "7140_meso",
