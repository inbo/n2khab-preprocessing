...
We check version **habitatmap_2025**.
...
There is one `<Null>` string in the INFO column.
...
```{r check-geometry}
# can run for a while! (40 min for version 2023, 7 min for version 2025)

# note that this could be made quicker with parallellization by working on
# chunks of this data (see purrr::in_parallel)
...
tm_shape(invalid_geoms %>% mutate(TAG = factor(TAG))) +
  tm_borders() +
  tm_facets(by = "TAG")
```
