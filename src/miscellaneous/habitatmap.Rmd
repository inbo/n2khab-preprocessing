...
if (length(grep("gpkg", list.files(path = filepath))) > 0) {
  ...
  habitatmap <- read_sf(filepath, layer = "habitatmap")
}
```
...
We convert all the field names to lowercase and remove all the underscores.
...
consequence of digitization errors. Note that there are also polygons that 
return `NA`.
