...
versions of the habitatmap without the need to modifiy every chunk)
...
There are many empty strings (apparently used instead of NA's), in most of the cases in the fields were empty values can be expected though.
...
These are the cases with spaces: 
...
As for tag, eenh2 and bigger, ...: there are too many of them to print the list.
...
habitatmap_lg <- habitatmap %>% 
  st_drop_geometry() %>% 
  select(tag, starts_with("hab")|starts_with("phab"), -any_of(c("HABLEGENDE", "hablegende"))) %>% 
