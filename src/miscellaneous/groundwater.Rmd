...
The one group with 11 piezometers:
...
result %>% 
    ...
    filter(n > 3) %>% 
    semi_join(result, .,
              by = "MeetpuntGroepCode") %>% 
    as_points(xvar = "MeetpuntXCoordinaat",
              yvar = "MeetpuntYCoordinaat") %>% 
    mapview::mapview(zcol = "MeetpuntGroepCode")
```

The locations of `Gr_WAR` are spread over > 1 km.

Interactive histogram of within-cluster distances (for not-yet clustered piezometers):

```{r warning=FALSE}
distances_unclustered <- 
  result %>% 
    filter(is.na(cluster_id),
           !is.na(MeetpuntXCoordinaat),
