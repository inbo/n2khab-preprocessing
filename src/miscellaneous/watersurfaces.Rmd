...
We check whether the structure of `WVLC` is correct.
...
```{r plot}
sf_fl <- read_admin_areas()
ggplot() +
  geom_sf(data = ws, aes(), color = "blue") + 
  geom_sf(data = sf_fl, fill = NA)
```

### Do all the watersurfaces intersect Flanders?

```{r in-fl}
in_fl <- st_intersects(x = sf_fl, y = ws)

not_in_flanders <- ws %>%
  st_drop_geometry() %>%
  mutate(orig_rowname = rownames(.)) %>%
  filter(!rownames(.) %in% in_fl[[1]])

nrow(not_in_flanders) == 0
```

```{r in-fl2, echo = FALSE}
if (nrow(not_in_flanders) > 0) {
  not_in_flanders %>%
  distinct(WVLC, NAAM) %>%
  kable()
}
