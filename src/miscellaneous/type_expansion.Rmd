...
```{r}
habitatmap_patches <- 
  read_habitatmap_stdized("../../n2khab_data") %>% 
  .$habitatmap_patches
```
...
Regarding **interpretation** of the habitatmap, which best goes into `habitatmap_terr` (the interpreted and augmented form of `habitatmap_stdized`) and `watersurfaces_interpr`:
...
So it will probably be better to apply this at once in the `habitatmap_terr` and in an interpreted form of `watersurfaces` (`watersurfaces_interpr`), which should be spatially exclusive (and together will be part of `habitatmap_integrated`).
