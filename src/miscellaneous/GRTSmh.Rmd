...
We could compare `raster::raster()`, `fasterize::fasterize()` and `stars::st_rasterize()`.
...
But "INT2U" is useless for the first bands of the brick, so we use "FLT4S".
...
```{r}
r2 <- raster(file.path(tempdir(), "GRTSmh_diffres.gpkg"))
r2 %>% spplot
```
...
```{r}
gdalUtils::gdalinfo(file.path(tempdir(), "GRTSmh_diffres.gpkg"))
```


Also `write_stars()` gets the GDAL error for a INT4S datatype.

```{r}
r %>% 
    st_as_stars %>% 
    write_stars(dsn = file.path(tempdir(), "GRTSmh_diffres.gpkg"),
                driver = "GPKG",
                type = "Int32")
```

Taking the default 'Float32', does work (FLT4S):
...
Now, let's add a different raster into the geopackage (currently `writeRaster()` does not support this):
...
```{r}
read_stars(file.path(tempdir(), "GRTSmh_diffres.gpkg"), sub = "GRTSmh_diffres")
```
...
```{r}
read_stars(file.path(tempdir(), "GRTSmh_diffres.gpkg"), sub = "lowres")
```


Hey, that geopackage is just about 3.8 MB!
What's the size of the geotiff? 5.6 MB, so the gpkg is more efficient with respect to file size.
