...
So, it seems better to:

- on the one hand, detect the watersurfaces containing aquatic habitats 31xx / 2190_a / (7220?) / rbbah, complemented by the habitatmap polygons with those types that aren't in `watersurfaces`.
This is a good basis for aquatic setups.
This is the aim of `watersurfaces_hab`.
A further interpreted form can be made, `watersurfaces_hab_interpr`, where code 3130 is interpreted as 3130_aom.
Or, no perhaps no distinction to make between both data sources and integrate that interpretation into `watersurfaces_hab`.
- on the other hand, delineate all other types (but still excluding 3260 and 1130) in `habitatmap_terr_interpr` which is useful for terrestrial + semi-terrestrial setups.
It contains the interpreted terrestrial + semi-terrestrial habitats.
To this aim, it has excluded aquatic types directly from `habitatmap_stdized`, and it further also excludes the _intersections_ between:
- watersurfaces that are labelled as '2190_a' (in `watersurfaces_hab`)
- polygons with label type '2190' (and nothing else) in the habitatmap.
