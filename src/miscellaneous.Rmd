...
See what happens with the default datatype 'FLT4S':

```{r}
sub4 <-
    sub10 %>% 
    calc(dec2base4,
        filename = "../data/20_processed/sub4.tif",
        # forcefun = TRUE,
        overwrite = TRUE)
```
...
```{r}
sub4 <- raster("../data/20_processed/sub4.tif")
sub4 %>% spplot
```
...
#### Working via a dataframe representation (OBSOLETE!)

_Note: this was still being done with the FLT4S datatype... However it shows that calculation and storage of such a dataframe is not as efficient as raster calculation (which went double as fast, on a single CPU core)._
...
### Rewinding: don't store base4 in rasters but in a dataframe

And yes, the stored version in a csv does work.
...
sub10[] %>% 
    ...
    mutate(base4frac = dec2base4(value)) %>% 
    (function(df) {
        write_csv(df, file.path(tempdir(), "grts_df.csv"))
        all.equal(df,
                  read_csv(file.path(tempdir(), "grts_df.csv"),
                           col_types = "idd"))
    })
```


However it may grow too large for the Flemish level.
...
GRTSmaster_habitats[] %>% 
    enframe %>% 
    mutate(base4frac = dec2base4(value)) %>% 
    write_csv(file.path("../data/20_processed", "GRTSmh_df.csv"))
```

I guess that took about 15 minutes, hum.

What if we try to read such thing, is that still doable?

```{r}
test <- 
    read_csv(file.path("../data/20_processed", "GRTSmh_df.csv"),
             col_types = "idd")
```

This took about 30 sec, so that's still doable. The object takes about 400 MB of RAM.

How large is the object in RData format?

```{r}
test %>% save(file = file.path("../data/20_processed", "GRTSmh_df.RData"))
```

This is ca. 178 MB in size, which is much smaller. It was written in about 30 sec.

Let's check the result.

```{r}
test %>% str(give.attr = FALSE)
test %>% 
    # filter(value >= 67108840)
    slice(base::sample.int(base::nrow(test), 30)) %>% 
    arrange(name)
```

This seems to be very OK.
