...
All of these need an extra row in watersurfaces_refpoints.
...
- Evolution of the watersurfaces data source: some polygons previously not distinguished in watersurfaces may already have been distinguished in previous watersurfaces_hab using the `polygon_id` coming from the habitatmap.
...
- If `polygon_id` values from _interim_ versions of watersurfaces_hab are included in the previous version of watersurfaces_refpoints (i.e. the previous version of watersurfaces_refpoints was an interim version), then that may also be a reason.
Given that we are creating a new regular version, note that we will also need to drop `polygon_id` values from the previous version that are specific to interim versions of watersurfaces_hab.
...
Checking whether (and which) mismatches would otherwise happen for illustration:
...
Before applying this in-polygon restriction in recycling existing reference points, we also want to make sure that no polygons of `wsh_new` actually do match a polygon from the previous watersurfaces_hab version with an out-of-polygon reference point.
...
Checking whether the point to be adopted is unique.
...
2. if multiple choices remain, use `polygon_id_previous` that is from the watersurfaces data source (it doesn't matter whether this is an interim polygon or not), or else the one that includes the oldest year (if year can be extracted);
3. if multiple choices remain, use the alphabetically first `polygon_id_previous`.
This will prefer regular `polygon_id_previous` values over interim ones (assuming that the latter begin with `"{"`).
...
wsh_new_point_exists <- 
  ...
  # following step deliberately only leads to non-NA values if all values of
  # year_hms_previous in a group are non-NA (so we use the default na.rm =
  # FALSE)
  mutate(
...
Generating the points (`refpoints_new`) to be added to the previous version of watersurfaces_refpoints.
...
## Constructing the current version of watersurfaces_refpoints

Combining previous and new refpoints; potentially dropping 'interim' `polygon_id` values if the previous version is an interim version.

Values for `polygon_id` can be accepted if they appear in `refpoints_last_regular` _or_ in the current version of watersurfaces_hab.

We should not forget that some values from the current version of watersurfaces_hab may already be present in `refpoints_previous` even if it is an interim version, so we must start from `refpoints_previous` in compiling `refpoints_current`.

```{r}
refpoints_all <- 
  refpoints_previous %>% 
  bind_rows(refpoints_new)
refpoints_last_regular <- 
  read_vc("watersurfaces_refpoints", root = refpoints_path_last_regular) %>% 
  as_tibble()
accepted_polygon_ids <- 
  refpoints_last_regular %>% 
  select(polygon_id) %>% 
  bind_rows(
...
Checking that all new refpoints have a `polygon_id` that can be accepted in a regular version.

```{r}
stopifnot(all(refpoints_new$polygon_id %in% accepted_polygon_ids))
```

Deriving the current (new) version of watersurfaces_refpoints by dropping the 'interim' `polygon_id` values from `refpoints_all`.

```{r}
refpoints_current <- 
  refpoints_all %>% 
  filter(polygon_id %in% accepted_polygon_ids) %>% 
  arrange(polygon_id)
```
...
Copying last regular version into target directory, so that overwriting it with git2rdata will minimize diffs between versions.

```{r}
file.copy(
  file.path(
    refpoints_path_last_regular,
